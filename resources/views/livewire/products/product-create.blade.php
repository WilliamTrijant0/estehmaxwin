<section>
    <div class="relative mb-6 w-full">
        <flux:heading size="xl" level="1">{{ __('Add Products') }}</flux:heading>
        <flux:subheading size="lg" class="mb-6">{{ __('Add new product') }}</flux:subheading>
        <flux:separator variant="subtle" />
    </div>

    <form wire:submit="storeProduct" class="flex flex-col gap-6">
        <!-- name -->
        ...
        <div>
            @if ($form->image)
                <img src="{{ $form->image->temporaryUrl() }}" alt="Image Preview" class="w-32 h-32 object-cover rounded">
            @endif
        </div>
